...
package uk.gov.hmrc.$serviceNameNoSpaceLowercase$.controllers.swagger
...
class ApiSpecsController @Inject()(
  ...
  val domainPackage = "uk.gov.hmrc.$serviceNameNoSpaceLowercase$"
  lazy val generator =
    com.iheart.playSwagger.SwaggerSpecGenerator(
      swaggerV3 = true,
      modelQualifier = PrefixDomainModelQualifier(domainPackage),
      apiVersion = Some(uk.gov.hmrc.$serviceNameNoSpaceLowercase$.BuildInfo.version))
